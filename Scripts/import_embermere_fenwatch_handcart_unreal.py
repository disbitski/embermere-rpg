"""Import and place Embermere's project-owned Fenwatch provision handcart."""
...
SOURCE_FBX = (
    PROJECT_ROOT
    / "ArtSource/Blender/Environment/FenwatchHandcart/"
    "SM_EmbermereFenwatchHandcart_01.fbx"
)
...
PLACEMENT_LABEL = "Embermere_FenwatchHandcart_CottageWest_01"
PLACEMENT_LOCATION = (-3040.0, -920.0, 0.0)
# The cart's handles point toward the cottage service edge. The placement stays
# west of the PlayerStart-to-Mara corridor and south of the cottage body.
PLACEMENT_YAW = 38.0
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch handcart integration failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 5436:
        fail("expected 5436 imported triangles, found {}".format(triangle_count))
    ...
    if abs(float(bounds.origin.z) - 97.25) > 1.0:
        fail("unexpected imported Z origin {}".format(bounds.origin.z))

    removed, created = place_handcart(mesh)
    unreal.log("Embermere Fenwatch handcart import paths: {}".format(imported_paths))
    unreal.log(
        "Embermere Fenwatch handcart material assignments: {}".format(assignments)
    )
    unreal.log(
        "Embermere Fenwatch handcart integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
            triangle_count,
            box_count,
            removed,
            created,
