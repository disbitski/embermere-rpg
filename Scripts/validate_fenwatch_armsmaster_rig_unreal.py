"""Validate saved rigged Fenwatch armsmaster packages and ownership."""
...
ART_ROOT = "/Game/Art/Embermere/Characters/NPCs/FenwatchArmsmaster"
STATIC_PATH = ART_ROOT + "/SM_EmbermereFenwatchArmsmaster_01"
SKELETAL_PATH = ART_ROOT + "/SK_EmbermereFenwatchArmsmaster_01"
IDLE_PATH = ART_ROOT + "/Animations/A_EmbermereFenwatchArmsmaster_Idle"
...
EXPECTED_MATERIALS = {
    "/Game/Art/Embermere/Characters/NPCs/FenwatchArmsmaster/M_FenwatchArmsmasterSkin.M_FenwatchArmsmasterSkin",
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch armsmaster rig validation failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if not isinstance(service, unreal.EmbermereTrainerServiceActor):
        fail("saved trainer service uses the wrong native class")
    ...
    unreal.log(
        "Embermere Fenwatch armsmaster rig validation passed: classic skeletal FBX packages, shared Skeleton and 3.2-second Idle, exact static fallback, one saved art-only skeletal presentation, and separate trainer authority"
    )
