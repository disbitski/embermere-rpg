"""Validate the saved Fenwatch handcart asset and map integration."""
...
EXPECTED_LOCATION = unreal.Vector(-3040.0, -920.0, 0.0)
EXPECTED_YAW = 38.0
EXPECTED_MATERIALS = {
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch handcart validation failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if mesh.get_num_triangles(0) != 5436:
    ...
    expected_dimensions = (430.578, 178.0, 194.5)
    actual_dimensions = (
    ...
    placements = []
    context = {}
    required_context = {COTTAGE_LABEL, MARA_LABEL, PLAYER_START_LABEL}
    for actor in actor_subsystem.get_all_level_actors():
        ...
        if label == PLACEMENT_LABEL:
            ...
            context[label] = actor
    if len(placements) != 1:
        fail("expected one saved handcart, found {}".format(len(placements)))
    ...
    minimums = {
        COTTAGE_LABEL: 700.0,
        MARA_LABEL: 950.0,
        PLAYER_START_LABEL: 600.0,
    }
    for label, minimum in minimums.items():
        if distances[label] < minimum:
            fail(
                "handcart moved too close to {}: {:.1f} cm".format(
                    label,
                    distances[label],
                )
            )
    ...
    if route_clearance < 650.0:
        fail(
            "handcart encroaches on the PlayerStart-to-Mara route: {:.1f} cm".format(
                route_clearance
            )
        )

    unreal.log(
        "Embermere Fenwatch handcart validation passed: 430.578 x 178 x 194.5 cm, 5436 triangles, five shared materials, four authored bed/axle/rest collision boxes, one grounded project-owned placement, presentation-only static art, and clear cottage, Mara, PlayerStart, and spawn-route spacing"
    )
