"""Log one read-only live-PIE sample of the Fenwatch quartermaster Idle clock."""
...
def fail(message):
    unreal.log_error("Embermere quartermaster Idle clock probe failed: {}".format(message))
    sys.exit(1)
...
def main():
    ...
    if len(matches) != 1:
        fail("expected one live quartermaster presentation, found {}".format(len(matches)))
    ...
    if position < 0.0 or position > 4.01:
    ...
    unreal.log(
        "Embermere quartermaster Idle clock sample: position={:.6f}, playing=true, "
