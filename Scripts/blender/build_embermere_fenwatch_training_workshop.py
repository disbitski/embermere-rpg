"""Build Embermere's original compact Fenwatch training workshop.

Run this reviewed scene-resetting script through Blender MCP in a factory-clean
headless process. The asset is an open-front training-yard shelter that uses the
shared Fenwatch stone, moss, timber, iron, and ember language. It exports four
purposeful UBX colliders, renders a preview, records deterministic metrics, and
saves the editable Blender source alongside the FBX.
"""
...
ASSET_NAME = "SM_EmbermereFenwatchTrainingWorkshop_01"
...
def add_collision() -> list[bpy.types.Object]:
    ...
    collision_specs = [
        ((34.0, 34.0, 246.0), (-184.0, 82.0, 147.0)),
        ((34.0, 34.0, 246.0), (184.0, 82.0, 147.0)),
        ((386.0, 26.0, 218.0), (0.0, -105.0, 154.0)),
        ((300.0, 58.0, 92.0), (0.0, -64.0, 70.0)),
    ]
    collision = []
...
def build_training_workshop() -> tuple[bpy.types.Object, list[bpy.types.Object]]:
    ...
    # Four compact masonry feet ground the structure while preserving the open
    # training-yard approach between the front posts.
    for side, x in (("Left", -184.0), ("Right", 184.0)):
        for lane, y in (("Front", 82.0), ("Rear", -96.0)):
            parts.extend(
                [
                    add_cylinder(
                        f"{side}{lane}StoneFoot",
                        8,
                        31.0,
                        20.0,
                        (x, y, 10.0),
    ...
    # A low stone-backed work wall gives the open shelter useful mass without
    # turning it into a closed building or hiding the practice dummy.
    parts.extend(
        [
            add_cube("RearStoneWall", (386.0, 26.0, 132.0), (0.0, -105.0, 88.0), stone, 4.0),
            add_cube("RearMossCap", (398.0, 34.0, 18.0), (0.0, -105.0, 158.0), moss, 3.0),
            add_cube("RearTimberPanel", (386.0, 20.0, 82.0), (0.0, -106.0, 211.0), timber, 3.0),
            add_cube("RearTopRail", (416.0, 34.0, 28.0), (0.0, -96.0, 278.0), timber, 4.0),
            add_cube("FrontHeader", (416.0, 34.0, 32.0), (0.0, 82.0, 278.0), timber, 4.0),
            add_cube("LeftSideHeader", (34.0, 212.0, 30.0), (-184.0, -7.0, 278.0), timber, 4.0),
            add_cube("RightSideHeader", (34.0, 212.0, 30.0), (184.0, -7.0, 278.0), timber, 4.0),
            add_cube("FrontBraceLeft", (105.0, 20.0, 20.0), (-139.0, 91.0, 239.0), timber, 2.0, (0.0, math.radians(-42.0), 0.0)),
            add_cube("FrontBraceRight", (105.0, 20.0, 20.0), (139.0, 91.0, 239.0), timber, 2.0, (0.0, math.radians(42.0), 0.0)),
        ]
    )

    # The solid rear bench and visual-only tool display sell the armsmaster's
    # purpose. Loose tools remain decorative so they never snag traversal.
    parts.extend(
        [
            add_cube("WorkbenchTop", (320.0, 68.0, 20.0), (0.0, -63.0, 108.0), timber, 4.0),
            add_cube("WorkbenchApron", (300.0, 18.0, 62.0), (0.0, -83.0, 73.0), timber, 3.0),
            add_cube("WorkbenchLeftLeg", (28.0, 48.0, 84.0), (-132.0, -64.0, 56.0), timber, 3.0),
            add_cube("WorkbenchRightLeg", (28.0, 48.0, 84.0), (132.0, -64.0, 56.0), timber, 3.0),
            add_cube("BenchIronBand", (322.0, 8.0, 10.0), (0.0, -29.0, 110.0), iron, 1.5),
            add_cube("ToolRail", (282.0, 12.0, 16.0), (0.0, -88.0, 213.0), iron, 2.0),
            add_cube("PracticeSwordBlade", (12.0, 10.0, 108.0), (-92.0, -76.0, 214.0), iron, 1.5, (0.0, math.radians(-16.0), 0.0)),
            add_cube("PracticeSwordGrip", (14.0, 12.0, 42.0), (-64.0, -73.0, 152.0), timber, 2.0, (0.0, math.radians(-16.0), 0.0)),
            add_cube("PracticeSwordGuard", (56.0, 12.0, 10.0), (-70.0, -70.0, 170.0), iron, 1.5, (0.0, math.radians(-16.0), 0.0)),
            add_cube("TrainingSpearShaft", (12.0, 10.0, 150.0), (91.0, -76.0, 198.0), timber, 1.5, (0.0, math.radians(11.0), 0.0)),
            add_cube("TrainingSpearHead", (25.0, 12.0, 40.0), (77.0, -73.0, 287.0), iron, 2.0, (0.0, math.radians(11.0), 0.0)),
        ]
    )

    # A broad moss gable, iron ridge, and small ember crest connect the module
    # to the existing Fenwatch shelter, stall, cottage, and roadside family.
    roof_angle = math.radians(27.0)
    parts.extend(
        [
            add_cube("RoofFront", (448.0, 150.0, 18.0), (0.0, 53.0, 317.0), moss, 3.0, (-roof_angle, 0.0, 0.0)),
            add_cube("RoofRear", (448.0, 150.0, 18.0), (0.0, -53.0, 317.0), moss, 3.0, (roof_angle, 0.0, 0.0)),
            add_cube("RoofRidge", (460.0, 22.0, 24.0), (0.0, 0.0, 357.0), iron, 3.0),
            add_cube("FrontFascia", (452.0, 18.0, 24.0), (0.0, 118.0, 282.0), timber, 3.0),
            add_cube("RearFascia", (452.0, 18.0, 24.0), (0.0, -118.0, 282.0), timber, 3.0),
            add_cylinder("FrontCrestIron", 12, 28.0, 9.0, (0.0, 126.0, 300.0), iron, (math.radians(90.0), 0.0, 0.0), 1.5),
            add_cylinder("FrontCrestStone", 12, 19.0, 10.0, (0.0, 132.0, 300.0), stone, (math.radians(90.0), 0.0, 0.0), 1.0),
            add_ember_gem("FrontCrestEmber", (0.0, 140.0, 300.0), (8.0, 4.0, 13.0), ember),
        ]
    )
...
def validate_asset(render_mesh: bpy.types.Object, collision: list[bpy.types.Object]) -> dict:
    ...
    if not (440.0 <= width <= 470.0 and 245.0 <= depth <= 275.0 and 355.0 <= height <= 380.0):
        errors.append(f"Unexpected dimensions: {metrics['dimensions_cm']}")
    if metrics["triangles"] > 10000:
    ...
    if errors:
        raise RuntimeError(
            "Embermere Fenwatch training workshop validation failed: "
            + "; ".join(errors)
        )
    return metrics
...
def add_preview_scene(render_mesh: bpy.types.Object, preview_path: Path) -> None:
    ...
    bpy.ops.object.light_add(type="AREA", location=(-430.0, 620.0, 500.0))
    key = bpy.context.object
    key.name = "Preview_Key"
    key.data.energy = 3100000.0
    key.data.shape = "DISK"
    key.data.size = 320.0
    look_at(key, (0.0, 0.0, 180.0))

    bpy.ops.object.light_add(type="AREA", location=(430.0, -340.0, 380.0))
    ...
    bpy.ops.object.light_add(type="POINT", location=(0.0, 175.0, 270.0))
    ...
    bpy.ops.object.camera_add(location=(760.0, 900.0, 540.0))
...
def main() -> dict:
    ...
    render_mesh, collision = build_training_workshop()
