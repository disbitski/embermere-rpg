"""Build Embermere's project-owned Fenwatch hitching trough.

Run this reviewed scene-resetting script through Blender MCP in a factory-clean
headless process. The low utility prop extends the west-cottage work cluster,
reuses the shared Fenwatch stone, moss, timber, iron, and ember language,
exports two purposeful UBX colliders, renders a preview, records deterministic
metrics, and saves editable Blender source.
"""
...
ASSET_NAME = "SM_EmbermereFenwatchHitchingTrough_01"
...
def resolve_output_paths(parameters: dict) -> tuple[Path, Path, Path, Path]:
    ...
    output_dir = Path(
        parameters.get(
            "output_dir",
            project_root / "ArtSource/Blender/Environment/FenwatchHitchingTrough",
        )
    ).expanduser()
...
def add_collision() -> list[bpy.types.Object]:
    ...
    collision_specs = [
        ((300.0, 102.0, 92.0), (0.0, 0.0, 67.0)),
        ((314.0, 26.0, 26.0), (0.0, 37.0, 150.0)),
    ]
    collision = []
...
def build_hitching_trough() -> tuple[bpy.types.Object, list[bpy.types.Object]]:
    ...
    # Four stone shoes and two timber runners ground the trough as a practical
    # village fixture while the broad authored body box owns its collision.
    for x in (-118.0, 118.0):
        for y in (-35.0, 35.0):
            parts.append(add_cube(f"StoneFoot_{x}_{y}", (42.0, 36.0, 16.0), (x, y, 8.0), stone, 3.0))
    parts.extend(
        [
            add_cube("LeftRunner", (34.0, 88.0, 24.0), (-118.0, 0.0, 27.0), timber, 3.0),
            add_cube("RightRunner", (34.0, 88.0, 24.0), (118.0, 0.0, 27.0), timber, 3.0),
            add_cube("TroughFloor", (270.0, 68.0, 18.0), (0.0, 0.0, 46.0), timber, 3.0),
            add_cube("FrontWall", (300.0, 20.0, 74.0), (0.0, -46.0, 78.0), timber, 4.0),
            add_cube("RearWall", (300.0, 20.0, 74.0), (0.0, 46.0, 78.0), timber, 4.0),
            add_cube("LeftEnd", (22.0, 104.0, 74.0), (-139.0, 0.0, 78.0), timber, 4.0),
            add_cube("RightEnd", (22.0, 104.0, 74.0), (139.0, 0.0, 78.0), timber, 4.0),
            add_cube("FeedBed", (258.0, 68.0, 10.0), (0.0, 0.0, 84.0), moss, 2.0),
        ]
    )

    # Iron straps and pins break up the broad timber silhouette. They remain
    # decorative detail inside the purposeful trough-body collision.
    for x in (-96.0, 96.0):
        parts.extend(
            [
                add_cube(f"FrontBand_{x}", (13.0, 7.0, 78.0), (x, -57.0, 78.0), iron, 1.0),
                add_cube(f"RearBand_{x}", (13.0, 7.0, 78.0), (x, 57.0, 78.0), iron, 1.0),
                add_cylinder(f"FrontPin_{x}", 10, 5.0, 7.0, (x, -62.0, 90.0), stone, (math.radians(90.0), 0.0, 0.0), 1.0),
            ]
        )

    # The rear hitching rail provides an unmistakable working-village read.
    # A second authored box covers the rail itself; caps, braces, and rings are
    # decorative so their thin forms cannot create collision snags.
    for side, x in (("Left", -132.0), ("Right", 132.0)):
        parts.extend(
            [
                add_cube(f"{side}HitchPost", (24.0, 24.0, 164.0), (x, 37.0, 94.0), timber, 3.0),
                add_cube(f"{side}PostCap", (36.0, 36.0, 16.0), (x, 37.0, 181.0), moss, 3.0),
                add_cube(
                    f"{side}Brace",
                    (18.0, 18.0, 82.0),
                    (x - (22.0 if side == "Left" else -22.0), 37.0, 115.0),
                    timber,
                    2.0,
                    (0.0, math.radians(-34.0 if side == "Left" else 34.0), 0.0),
                ),
            ]
        )
    parts.extend(
        [
            add_cube("HitchRail", (314.0, 24.0, 24.0), (0.0, 37.0, 150.0), timber, 3.0),
            add_cube("RailIronTrim", (286.0, 7.0, 8.0), (0.0, 22.0, 150.0), iron, 1.0),
        ]
    )

    for index, x in enumerate((-82.0, 0.0, 82.0), start=1):
        parts.append(
            add_torus(
                f"HitchRing_{index:02d}",
                13.0,
                3.5,
                (x, 20.0, 141.0),
    ...
    # A small front crest repeats Fenwatch's visual signature without implying
    # that this presentation-only prop owns an interaction or service.
    parts.extend(
        [
            add_cylinder("FrontCrestIron", 12, 18.0, 8.0, (0.0, -58.0, 78.0), iron, (math.radians(90.0), 0.0, 0.0), 1.0),
            add_cylinder("FrontCrestStone", 12, 11.0, 9.0, (0.0, -63.0, 78.0), stone, (math.radians(90.0), 0.0, 0.0), 1.0),
            add_ember_gem("FrontCrestEmber", (0.0, -70.0, 78.0), (6.0, 4.0, 6.0), ember),
        ]
    )
...
def validate_asset(render_mesh: bpy.types.Object, collision: list[bpy.types.Object]) -> dict:
    ...
    if not (350.0 <= width <= 380.0 and 120.0 <= depth <= 150.0 and 180.0 <= height <= 205.0):
    ...
    if errors:
        raise RuntimeError(
            "Embermere Fenwatch hitching-trough validation failed: "
            + "; ".join(errors)
        )
    return metrics
...
def add_preview_scene(render_mesh: bpy.types.Object, preview_path: Path) -> None:
    ...
    bpy.ops.object.light_add(type="AREA", location=(-300.0, 430.0, 360.0))
    key = bpy.context.object
    key.name = "Preview_Key"
    key.data.energy = 2200000.0
    key.data.shape = "DISK"
    key.data.size = 260.0
    look_at(key, (0.0, 0.0, 90.0))

    bpy.ops.object.light_add(type="AREA", location=(430.0, -310.0, 250.0))
    fill = bpy.context.object
    fill.name = "Preview_Fill"
    fill.data.energy = 950000.0
    fill.data.color = (0.35, 0.5, 0.78)
    fill.data.size = 230.0
    look_at(fill, (0.0, 0.0, 85.0))

    bpy.ops.object.light_add(type="POINT", location=(0.0, -90.0, 78.0))
    glow = bpy.context.object
    glow.name = "Preview_EmberGlow"
    glow.data.energy = 42000.0
    glow.data.color = (1.0, 0.16, 0.025)
    glow.data.shadow_soft_size = 34.0

    bpy.ops.object.camera_add(location=(440.0, -470.0, 260.0))
    camera = bpy.context.object
    camera.name = "Preview_Camera"
    camera.data.lens = 60.0
    camera.data.clip_end = 3000.0
    look_at(camera, (0.0, 0.0, 82.0))
...
def main() -> dict:
    ...
    render_mesh, collision = build_hitching_trough()
