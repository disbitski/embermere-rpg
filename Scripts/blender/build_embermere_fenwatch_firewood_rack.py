"""Build Embermere's project-owned Fenwatch firewood rack.

Run this reviewed scene-resetting script through Blender MCP in a factory-clean
headless process. The rack reuses the shared Fenwatch stone, moss, timber,
...
ASSET_NAME = "SM_EmbermereFenwatchFirewoodRack_01"
...
def resolve_output_paths(parameters: dict) -> tuple[Path, Path, Path, Path]:
    ...
    output_dir = Path(
        parameters.get(
            "output_dir",
            project_root / "ArtSource/Blender/Environment/FenwatchFirewoodRack",
        )
    ).expanduser()
...
def add_log(
    ...
    log = add_cylinder(name, 10, radius, length, (x, y, z), timber, rotation)
    band = add_cylinder(
        f"{name}EndGrain",
        10,
        radius * 0.72,
        1.5,
        (x - length * 0.5 - 0.8, y, z),
        iron,
        rotation,
    )
    return [log, band]
...
def add_collision() -> list[bpy.types.Object]:
    ...
    collision_specs = [
        ((228.0, 82.0, 112.0), (-18.0, 8.0, 60.0)),
        ((26.0, 112.0, 196.0), (-126.0, 4.0, 98.0)),
        ((26.0, 112.0, 196.0), (90.0, 4.0, 98.0)),
        ((58.0, 58.0, 72.0), (174.0, -24.0, 36.0)),
    ]
    collision = []
...
def build_firewood_rack() -> tuple[bpy.types.Object, list[bpy.types.Object]]:
    ...
    # A stone-footed lean-to frame makes the firewood read as cottage utility
    # architecture rather than a loose collision pile.
    for side, x in (("Left", -126.0), ("Right", 90.0)):
        for depth_name, y in (("Front", -42.0), ("Rear", 50.0)):
            parts.extend(
                [
                    add_cube(
                        f"{side}{depth_name}Foot",
                        (40.0, 40.0, 14.0),
                        (x, y, 7.0),
                        stone,
                        3.0,
    ...
    parts.extend(
        [
            add_cube("FrontBeam", (256.0, 20.0, 24.0), (-18.0, -42.0, 194.0), timber, 2.0),
            add_cube("RearBeam", (256.0, 20.0, 24.0), (-18.0, 50.0, 194.0), timber, 2.0),
            add_cube("LeftBrace", (20.0, 105.0, 18.0), (-126.0, 4.0, 154.0), timber, 1.5, (math.radians(-18.0), 0.0, 0.0)),
            add_cube("RightBrace", (20.0, 105.0, 18.0), (90.0, 4.0, 154.0), timber, 1.5, (math.radians(-18.0), 0.0, 0.0)),
            add_cube("RackFloor", (232.0, 88.0, 14.0), (-18.0, 8.0, 24.0), timber, 2.0),
        ]
    )

    # Two shallow moss-roof planes meet at a central ridge. They are purely
    # decorative so players cannot collide with an invisible overhang.
    roof_pitch = math.radians(14.0)
    parts.extend(
        [
            add_cube(
                "FrontRoof",
            ...
            add_cube("RoofRidge", (306.0, 14.0, 18.0), (-18.0, 7.0, 229.0), timber, 2.0),
            add_cube("FrontIronTrim", (280.0, 7.0, 10.0), (-18.0, -70.0, 209.0), iron, 1.0),
        ]
    )

    # Layered logs stay individually readable, while one authored box owns the
    # broad collision expected from the stacked mass.
    log_rows = (
        (-80.0, -22.0, 43.0, 12.0),
        (-22.0, -18.0, 43.0, 11.0),
        (38.0, -21.0, 43.0, 12.5),
        ...
        (-43.0, 5.0, 97.0, 11.5),
        (22.0, 7.0, 98.0, 12.0),
        (-24.0, 28.0, 120.0, 11.0),
        (38.0, 27.0, 119.0, 10.5),
    )
    for index, (x, y, z, radius) in enumerate(log_rows):
        parts.extend(
            add_log(
                f"FirewoodLog{index + 1:02d}",
                x,
                y,
                z,
                radius,
    ...
    # The chopping block is purposeful solid mass; the planted axe remains
    # decorative to avoid a thin snag in the cottage-side route.
    parts.extend(
        [
            add_cylinder("ChoppingBlock", 12, 28.0, 70.0, (174.0, -24.0, 35.0), timber, bevel=2.0),
            add_cube("AxeHandle", (12.0, 12.0, 112.0), (188.0, -23.0, 97.0), timber, 1.5, (0.0, math.radians(-11.0), math.radians(4.0))),
            add_cube("AxeHead", (42.0, 12.0, 26.0), (199.0, -22.0, 150.0), iron, 2.0, (0.0, math.radians(-11.0), math.radians(4.0))),
        ]
    )

    # The restrained crest repeats the village signature without suggesting
    # that the presentation-only prop can be interacted with.
    parts.extend(
        [
            add_cylinder("FrontCrestIron", 12, 18.0, 8.0, (-18.0, -58.0, 185.0), iron, (math.radians(90.0), 0.0, 0.0), 1.0),
            add_cylinder("FrontCrestStone", 12, 11.0, 9.0, (-18.0, -63.0, 185.0), stone, (math.radians(90.0), 0.0, 0.0), 1.0),
            add_ember_gem("FrontCrestEmber", (-18.0, -70.0, 185.0), (6.0, 4.0, 6.0), ember),
        ]
    )
...
def validate_asset(render_mesh: bpy.types.Object, collision: list[bpy.types.Object]) -> dict:
    ...
    if not (350.0 <= width <= 430.0 and 140.0 <= depth <= 180.0 and 220.0 <= height <= 270.0):
    ...
    if errors:
        raise RuntimeError(
            "Embermere Fenwatch firewood-rack validation failed: "
            + "; ".join(errors)
        )
    return metrics
...
def add_preview_scene(render_mesh: bpy.types.Object, preview_path: Path) -> None:
    ...
    bpy.ops.mesh.primitive_plane_add(size=720.0, location=(10.0, 0.0, -0.5))
    ...
    bpy.ops.object.light_add(type="POINT", location=(-18.0, -90.0, 185.0))
    glow = bpy.context.object
    glow.name = "Preview_EmberGlow"
    glow.data.energy = 45000.0
    glow.data.color = (1.0, 0.16, 0.025)
    glow.data.shadow_soft_size = 38.0

    bpy.ops.object.camera_add(location=(470.0, -500.0, 300.0))
    camera = bpy.context.object
    camera.name = "Preview_Camera"
    camera.data.lens = 58.0
    camera.data.clip_end = 3000.0
    look_at(camera, (5.0, 0.0, 105.0))
...
def main() -> dict:
    ...
    render_mesh, collision = build_firewood_rack()
