"""Build Embermere's original open-sided Fenwatch village shelter.

Run this reviewed script through Blender MCP with inline code disabled. It
creates a reusable Stylized Classic quest/vendor shelter, authors four support
colliders, renders a preview, exports classic-FBX-compatible source, records
deterministic metrics, and saves the editable Blender file.
"""
...
def resolve_output_paths(parameters: dict) -> tuple[Path, Path, Path, Path]:
    ...
    output_dir = Path(
        parameters.get(
            "output_dir",
            project_root / "ArtSource/Blender/Environment/FenwatchShelter",
        )
    ).expanduser()
...
def add_collision() -> list[bpy.types.Object]:
    ...
    for index, (x, y) in enumerate(
        [(-160.0, -88.0), (160.0, -88.0), (-160.0, 88.0), (160.0, 88.0)]
    ):
        collision.append(
            add_cube(
...
def build_shelter() -> tuple[bpy.types.Object, list[bpy.types.Object]]:
    ...
    # Four stone-footed timber supports keep the pavilion open and traversable.
    for side, x in (("L", -160.0), ("R", 160.0)):
        for row, y in (("F", -88.0), ("B", 88.0)):
            parts.extend(
                [
                    add_cube(
                        f"StoneFoot_{side}{row}",
                        (62.0, 62.0, 36.0),
                        (x, y, 18.0),
                        stone,
                        4.0,
    ...
    # Structural beams and diagonal braces establish the roadside family.
    parts.extend(
        [
            add_cube("FrontBeam", (374.0, 28.0, 30.0), (0.0, -88.0, 282.0), timber, 4.0),
            add_cube("BackBeam", (374.0, 28.0, 30.0), (0.0, 88.0, 282.0), timber, 4.0),
            add_cube("LeftTie", (28.0, 204.0, 27.0), (-160.0, 0.0, 282.0), timber, 4.0),
            add_cube("RightTie", (28.0, 204.0, 27.0), (160.0, 0.0, 282.0), timber, 4.0),
            add_cube("FrontBraceL", (96.0, 18.0, 18.0), (-119.0, -89.0, 244.0), timber, 2.0, (0.0, math.radians(-43.0), 0.0)),
            add_cube("FrontBraceR", (96.0, 18.0, 18.0), (119.0, -89.0, 244.0), timber, 2.0, (0.0, math.radians(43.0), 0.0)),
            add_cube("BackBraceL", (96.0, 18.0, 18.0), (-119.0, 89.0, 244.0), timber, 2.0, (0.0, math.radians(-43.0), 0.0)),
            add_cube("BackBraceR", (96.0, 18.0, 18.0), (119.0, 89.0, 244.0), timber, 2.0, (0.0, math.radians(43.0), 0.0)),
        ]
    )

    # Two broad roof planes form a readable gable without interior collision.
    roof_angle = math.radians(25.0)
    parts.extend(
        [
            add_cube("RoofFront", (424.0, 176.0, 18.0), (0.0, -66.0, 319.0), timber, 3.0, (roof_angle, 0.0, 0.0)),
            add_cube("RoofBack", (424.0, 176.0, 18.0), (0.0, 66.0, 319.0), timber, 3.0, (-roof_angle, 0.0, 0.0)),
            add_cube("RoofRidge", (438.0, 24.0, 25.0), (0.0, 0.0, 357.0), stone, 4.0),
            add_cube("MossFront", (286.0, 58.0, 5.0), (-34.0, -38.0, 344.0), moss, 2.0, (roof_angle, 0.0, 0.0)),
            add_cube("MossBack", (230.0, 52.0, 5.0), (48.0, 38.0, 344.0), moss, 2.0, (-roof_angle, 0.0, 0.0)),
        ]
    )

    # A low rear rail gives the silhouette a village function without closing it.
    parts.extend(
        [
            add_cube("RearRailTop", (292.0, 20.0, 20.0), (0.0, 88.0, 116.0), timber, 3.0),
            add_cube("RearRailLow", (292.0, 18.0, 18.0), (0.0, 88.0, 61.0), timber, 3.0),
            add_cube("RearRailPostL", (19.0, 19.0, 92.0), (-95.0, 88.0, 78.0), timber, 2.0),
            add_cube("RearRailPostC", (19.0, 19.0, 92.0), (0.0, 88.0, 78.0), timber, 2.0),
            add_cube("RearRailPostR", (19.0, 19.0, 92.0), (95.0, 88.0, 78.0), timber, 2.0),
        ]
    )

    # The front crest carries Embermere's pale-stone, iron, and ember identity.
    parts.extend(
        [
            add_cylinder(
                "FrontCrestStone",
            ...
            add_ember_gem("FrontCrestEmber", (0.0, -128.0, 282.0), (10.0, 6.0, 17.0), ember),
            add_cube("FrontIronBracketL", (12.0, 34.0, 54.0), (-155.0, -95.0, 278.0), iron, 2.0),
            add_cube("FrontIronBracketR", (12.0, 34.0, 54.0), (155.0, -95.0, 278.0), iron, 2.0),
        ]
    )
...
def validate_asset(render_mesh: bpy.types.Object, collision: list[bpy.types.Object]) -> dict:
    ...
    if not (420.0 <= width <= 450.0 and 285.0 <= depth <= 305.0 and 350.0 <= height <= 380.0):
    ...
    if errors:
        raise RuntimeError("Embermere Fenwatch shelter validation failed: " + "; ".join(errors))
    return metrics
...
def add_preview_scene(render_mesh: bpy.types.Object, preview_path: Path) -> None:
    ...
    bpy.ops.object.light_add(type="AREA", location=(-420.0, -520.0, 430.0))
    key = bpy.context.object
    key.name = "Preview_Key"
    key.data.energy = 3100000.0
    key.data.shape = "DISK"
    key.data.size = 310.0
    look_at(key, (0.0, 0.0, 170.0))

    bpy.ops.object.light_add(type="AREA", location=(380.0, 260.0, 340.0))
    fill = bpy.context.object
    fill.name = "Preview_Fill"
    fill.data.energy = 1400000.0
    fill.data.color = (0.35, 0.5, 0.78)
    fill.data.size = 280.0
    look_at(fill, (0.0, 0.0, 160.0))

    bpy.ops.object.light_add(type="POINT", location=(0.0, -150.0, 288.0))
    ...
    bpy.ops.object.camera_add(location=(520.0, -650.0, 420.0))
    camera = bpy.context.object
    camera.name = "Preview_Camera"
    camera.data.lens = 58.0
    look_at(camera, (0.0, 0.0, 170.0))
...
def main() -> dict:
    ...
    render_mesh, collision = build_shelter()
