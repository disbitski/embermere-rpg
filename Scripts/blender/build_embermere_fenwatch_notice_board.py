"""Build Embermere's project-owned Fenwatch roadside notice board.

Run this reviewed scene-resetting script through Blender MCP in a factory-clean
headless process. The asset uses the shared Fenwatch stone, moss, timber, iron,
and ember language, exports three purposeful UBX colliders, renders a preview,
records deterministic metrics, and saves the editable Blender source with FBX.
"""
...
ASSET_NAME = "SM_EmbermereFenwatchNoticeBoard_01"
...
def resolve_output_paths(parameters: dict) -> tuple[Path, Path, Path, Path]:
    ...
    output_dir = Path(
        parameters.get(
            "output_dir",
            project_root / "ArtSource/Blender/Environment/FenwatchNoticeBoard",
        )
    ).expanduser()
...
def add_collision() -> list[bpy.types.Object]:
    ...
    collision_specs = [
        ((54.0, 54.0, 224.0), (-94.0, 0.0, 112.0)),
        ((54.0, 54.0, 224.0), (94.0, 0.0, 112.0)),
        ((242.0, 24.0, 116.0), (0.0, 0.0, 169.0)),
    ]
    collision = []
...
def build_notice_board() -> tuple[bpy.types.Object, list[bpy.types.Object]]:
    ...
    # Stone feet, moss courses, and timber uprights provide the only low-level
    # collision. Their broad stance keeps the board readable beside the road.
    ...
    # The board is deliberately pictographic: layered blank parchment shapes
    # and pins suggest notices without generated text or gameplay promises.
    parts.extend(
        [
            add_cube("NoticeBoard", (242.0, 20.0, 116.0), (0.0, 0.0, 169.0), timber, 4.0),
            add_cube("TopFrame", (266.0, 30.0, 18.0), (0.0, 0.0, 232.0), timber, 3.0),
            add_cube("BottomFrame", (266.0, 30.0, 18.0), (0.0, 0.0, 106.0), timber, 3.0),
            add_cube("LeftFrame", (18.0, 30.0, 116.0), (-124.0, 0.0, 169.0), timber, 3.0),
            add_cube("RightFrame", (18.0, 30.0, 116.0), (124.0, 0.0, 169.0), timber, 3.0),
            add_cube("NoticeLarge", (78.0, 3.5, 74.0), (-67.0, 12.0, 171.0), stone, 1.5, (0.0, math.radians(-4.0), math.radians(-3.0))),
            add_cube("NoticeUpper", (72.0, 3.5, 34.0), (45.0, 12.0, 191.0), stone, 1.5, (0.0, math.radians(3.0), math.radians(2.0))),
            add_cube("NoticeLower", (66.0, 3.5, 32.0), (55.0, 12.0, 145.0), stone, 1.5, (0.0, math.radians(-2.0), math.radians(-2.0))),
            add_cube("NoticeStrip", (46.0, 3.5, 19.0), (-8.0, 12.0, 122.0), moss, 1.5, (0.0, 0.0, math.radians(4.0))),
        ]
    )

    for index, (x, z) in enumerate(((-88.0, 198.0), (-45.0, 144.0), (20.0, 204.0), (73.0, 166.0))):
        parts.append(
            add_cylinder(
                f"NoticePin{index + 1}",
                12,
    ...
    # A compact moss roof and ember crest tie this utilitarian prop to the
    # shelter, stall, cottage, gate, and roadside family without adding snags.
    roof_angle = math.radians(28.0)
    parts.extend(
        [
            add_cube("RoofFront", (284.0, 52.0, 16.0), (0.0, 19.0, 251.0), moss, 3.0, (-roof_angle, 0.0, 0.0)),
            add_cube("RoofRear", (284.0, 52.0, 16.0), (0.0, -19.0, 251.0), moss, 3.0, (roof_angle, 0.0, 0.0)),
            add_cube("RoofRidge", (286.0, 18.0, 18.0), (0.0, 0.0, 268.0), iron, 2.0),
            add_cube("LeftBrace", (70.0, 16.0, 16.0), (-70.0, -1.0, 239.0), timber, 2.0, (0.0, math.radians(-38.0), 0.0)),
            add_cube("RightBrace", (70.0, 16.0, 16.0), (70.0, -1.0, 239.0), timber, 2.0, (0.0, math.radians(38.0), 0.0)),
            add_cylinder("CrestIron", 12, 25.0, 8.0, (0.0, 34.0, 245.0), iron, (math.radians(90.0), 0.0, 0.0), 1.5),
            add_cylinder("CrestStone", 12, 17.0, 9.0, (0.0, 39.0, 245.0), stone, (math.radians(90.0), 0.0, 0.0), 1.0),
            add_ember_gem("CrestEmber", (0.0, 46.0, 245.0), (7.0, 4.0, 11.0), ember),
        ]
    )
...
def validate_asset(render_mesh: bpy.types.Object, collision: list[bpy.types.Object]) -> dict:
    ...
    if not (280.0 <= width <= 300.0 and 80.0 <= depth <= 100.0 and 270.0 <= height <= 290.0):
    ...
    if errors:
        raise RuntimeError(
            "Embermere Fenwatch notice board validation failed: " + "; ".join(errors)
        )
    return metrics
...
def add_preview_scene(render_mesh: bpy.types.Object, preview_path: Path) -> None:
    ...
    bpy.ops.object.light_add(type="AREA", location=(-330.0, 480.0, 380.0))
    key = bpy.context.object
    key.name = "Preview_Key"
    key.data.energy = 2400000.0
    key.data.shape = "DISK"
    key.data.size = 260.0
    look_at(key, (0.0, 0.0, 140.0))

    bpy.ops.object.light_add(type="AREA", location=(310.0, -260.0, 300.0))
    fill = bpy.context.object
    fill.name = "Preview_Fill"
    fill.data.energy = 1000000.0
    fill.data.color = (0.35, 0.5, 0.78)
    fill.data.size = 230.0
    look_at(fill, (0.0, 0.0, 140.0))

    bpy.ops.object.light_add(type="POINT", location=(0.0, 90.0, 235.0))
    ...
    bpy.ops.object.camera_add(location=(430.0, 560.0, 325.0))
    camera = bpy.context.object
    camera.name = "Preview_Camera"
    camera.data.lens = 58.0
    camera.data.clip_end = 3000.0
    look_at(camera, (0.0, 0.0, 135.0))
...
def main() -> dict:
    ...
    render_mesh, collision = build_notice_board()
