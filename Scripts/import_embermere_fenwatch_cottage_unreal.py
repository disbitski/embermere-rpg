"""Import and place Embermere's project-owned Fenwatch cottage."""
...
SOURCE_FBX = (
    PROJECT_ROOT
    / "ArtSource/Blender/Environment/FenwatchCottage/"
    "SM_EmbermereFenwatchCottage_01.fbx"
)
...
PLACEMENT_LOCATION = (-2480.0, -260.0, 0.0)
# Classic FBX mirrors Blender's local Y. This yaw turns the closed front toward
# Mara and the village while keeping the cottage north of the spawn route.
PLACEMENT_YAW = 38.0
...
def fail(message):
    unreal.log_error("Embermere Fenwatch cottage integration failed: {}".format(message))
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 6616:
        fail("expected 6616 imported triangles, found {}".format(triangle_count))
    ...
    unreal.log("Embermere Fenwatch cottage import paths: {}".format(imported_paths))
    unreal.log("Embermere Fenwatch cottage material assignments: {}".format(assignments))
    unreal.log(
        "Embermere Fenwatch cottage integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
            triangle_count,
            box_count,
            removed,
            created,
