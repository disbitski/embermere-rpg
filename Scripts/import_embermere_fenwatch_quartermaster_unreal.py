"""Import and place Embermere's presentation-only Fenwatch quartermaster."""
...
SOURCE_FBX = (
    ...
    "SM_EmbermereFenwatchQuartermaster_01.fbx"
)
...
PLACEMENT_LABEL = "Embermere_FenwatchQuartermaster_Vendor_01"
PLACEMENT_LOCATION = (-1530.0, -1190.0, 0.0)
PLACEMENT_YAW = 100.0
...
def fail(message):
    unreal.log_error("Embermere Fenwatch quartermaster integration failed: {}".format(message))
    sys.exit(1)
...
def import_static_mesh():
    ...
    # Keep the proven classic importer. This presentation mesh intentionally
    # carries no collision; interaction remains on a separate gameplay actor.
...
def assign_materials(mesh):
    ...
    if not skin or not isinstance(skin, unreal.Material):
        fail("import did not produce the project-owned quartermaster skin material")
...
def validate_mesh(mesh):
    ...
    if collision_count != 0:
        fail("quartermaster must remain non-colliding, found {} shapes".format(collision_count))
    ...
    if not all((
        ...
        fail("quartermaster bounds drifted: origin={}, extent={}".format(
            bounds.origin,
            bounds.box_extent,
        ))

    triangle_count = mesh.get_num_triangles(0)
    if triangle_count != 3632:
        fail("expected 3632 triangles, found {}".format(triangle_count))
    return collision_count, size, triangle_count


def place_quartermaster(mesh):
    ...
    if actor.get_resolved_visual_mode() != unreal.EmbermereNpcVisualMode.STATIC_MESH:
        fail("quartermaster did not resolve to the static presentation lane")
...
def main():
    ...
    removed, created, tags = place_quartermaster(mesh)
    unreal.log("Embermere Fenwatch quartermaster import paths: {}".format(imported_paths))
    unreal.log("Embermere Fenwatch quartermaster material assignments: {}".format(assignments))
    unreal.log(
        "Embermere Fenwatch quartermaster integration passed: triangles={}, dimensions={}, collision={}, removed={}, created={}, tags={}".format(
            triangle_count,
            size,
            collision_count,
            removed,
