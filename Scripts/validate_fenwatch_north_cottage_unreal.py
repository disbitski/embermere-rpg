"""Validate the saved Fenwatch north cottage asset and map integration."""
...
PLACEMENT_LABEL = "Embermere_FenwatchCottage_North_01"
...
EXPECTED_LOCATION = unreal.Vector(-2500.0, 680.0, 0.0)
EXPECTED_YAW = 28.0
EXPECTED_MATERIALS = {
...
def fail(message):
    unreal.log_error("Embermere Fenwatch north cottage validation failed: {}".format(message))
    sys.exit(1)
...
def main():
    ...
    if abs(float(bounds.origin.z) - 236.0) > 1.0:
    ...
    for actor in actor_subsystem.get_all_level_actors():
        ...
        elif label in {MARA_LABEL, PLAYER_START_LABEL, WEST_COTTAGE_LABEL, HANDCART_LABEL}:
            context[label] = actor
    if len(placements) != 1:
        fail("expected one saved cottage, found {}".format(len(placements)))
    for required in {MARA_LABEL, PLAYER_START_LABEL, WEST_COTTAGE_LABEL, HANDCART_LABEL}:
    ...
    if mara_distance < 1400.0:
        fail("cottage moved too close to Mara's marker/greeting lane: {:.1f} cm".format(mara_distance))

    west_cottage = context[WEST_COTTAGE_LABEL].get_actor_location()
    cottage_separation = math.hypot(
        EXPECTED_LOCATION.x - west_cottage.x,
        EXPECTED_LOCATION.y - west_cottage.y,
    )
    if cottage_separation < 900.0:
        fail("north cottage moved too close to the west cottage: {:.1f} cm".format(cottage_separation))

    handcart = context[HANDCART_LABEL].get_actor_location()
    handcart_separation = math.hypot(
        EXPECTED_LOCATION.x - handcart.x,
        EXPECTED_LOCATION.y - handcart.y,
    )
    if handcart_separation < 1500.0:
        fail("north cottage moved too close to the handcart approach: {:.1f} cm".format(handcart_separation))

    unreal.log(
        "Embermere Fenwatch north cottage validation passed: 550 x 497.403 x 472 cm, 7132 triangles, five shared materials, six authored body/porch/step/post collision boxes, one grounded north-village placement, {:.1f} cm spawn-route clearance, {:.1f} cm Mara separation, {:.1f} cm cottage-center separation, and {:.1f} cm handcart separation".format(
            route_clearance,
            mara_distance,
            cottage_separation,
            handcart_separation,
