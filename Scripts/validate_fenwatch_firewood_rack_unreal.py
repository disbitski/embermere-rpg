"""Validate the saved Fenwatch firewood-rack asset and map integration."""
...
EXPECTED_LOCATION = unreal.Vector(-3050.0, -120.0, 0.0)
EXPECTED_YAW = 38.0
EXPECTED_MATERIALS = {
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch firewood-rack validation failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if mesh.get_num_triangles(0) != 3644:
    ...
    expected_dimensions = (392.937, 157.793, 238.0)
    actual_dimensions = (
    ...
    for actor in actor_subsystem.get_all_level_actors():
        ...
        if label == PLACEMENT_LABEL:
            ...
            context[label] = actor
    if len(placements) != 1:
        fail("expected one saved firewood rack, found {}".format(len(placements)))
    ...
    minimums = {
        COTTAGE_LABEL: 560.0,
        HANDCART_LABEL: 750.0,
        MARA_LABEL: 1100.0,
        PLAYER_START_LABEL: 1000.0,
    }
    for label, minimum in minimums.items():
        if distances[label] < minimum:
            fail(
                "firewood rack moved too close to {}: {:.1f} cm".format(
                    label,
                    distances[label],
                )
            )
    ...
    if route_clearance < 850.0:
        fail(
            "firewood rack encroaches on the PlayerStart-to-Mara route: {:.1f} cm".format(
                route_clearance
            )
        )

    unreal.log(
        "Embermere Fenwatch firewood-rack validation passed: 392.937 x 157.793 x 238 cm, 3644 triangles, five shared materials, four authored stack/support/block collision boxes, one grounded project-owned placement, presentation-only static art, and clear cottage, handcart, Mara, PlayerStart, and spawn-route spacing"
    )
