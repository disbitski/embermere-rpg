"""Import Embermere's supply chest and replace a temporary vendor crate stack."""
...
SOURCE_FBX = PROJECT_ROOT / "ArtSource/Blender/Environment/SupplyChest/SM_EmbermereSupplyChest_01.fbx"
...
PLACEMENT_LABEL = "Embermere_SupplyChest_Vendor_01"
PLACEMENT_LOCATION = (-1545.0, -920.0, 20.0)
PLACEMENT_YAW = 108.0
PLACEMENT_FOLDER = "04_Fab_Zone_Pass/01_Village/Embermere_Originals"
REPLACED_FAB_LABEL = "FabPass_Village_Crates_A"
...
def fail(message):
    unreal.log_error("Embermere supply chest integration failed: {}".format(message))
    sys.exit(1)
...
def import_static_mesh():
    ...
    # The classic factory is mandatory for authored UBX collision in UE 5.8.
...
def main():
    ...
    if abs(float(bounds.origin.z) - 61.55) > 2.0 or abs(float(bounds.box_extent.z) - 61.55) > 2.0:
        fail("unexpected imported Z bounds origin={} extent={}".format(bounds.origin.z, bounds.box_extent.z))

    removed, created = place_supply_chest(mesh)
    unreal.log("Embermere supply chest import paths: {}".format(imported_paths))
    unreal.log("Embermere supply chest material assignments: {}".format(assignments))
    unreal.log(
        "Embermere supply chest integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
