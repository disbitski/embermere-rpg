"""Import and place Embermere's project-owned Fenwatch training workshop."""
...
SOURCE_FBX = (
    ...
    "SM_EmbermereFenwatchTrainingWorkshop_01.fbx"
)
...
PLACEMENT_LOCATION = (-690.0, -1030.0, 0.0)
# Classic FBX mirrors Blender's local Y. This yaw turns the open front toward
# the armsmaster and practice dummy while keeping the road-side bypass clear.
PLACEMENT_YAW = -100.0
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch training workshop integration failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 5624:
        fail("expected 5624 imported triangles, found {}".format(triangle_count))
    ...
    removed, created = place_workshop(mesh)
    unreal.log("Embermere Fenwatch training workshop import paths: {}".format(imported_paths))
    unreal.log(
        "Embermere Fenwatch training workshop material assignments: {}".format(
            assignments
        )
    )
    unreal.log(
        "Embermere Fenwatch training workshop integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
            triangle_count,
            box_count,
            removed,
            created,
