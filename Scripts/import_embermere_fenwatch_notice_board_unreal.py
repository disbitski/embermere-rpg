"""Import and place Embermere's project-owned Fenwatch notice board."""
...
SOURCE_FBX = (
    PROJECT_ROOT
    / "ArtSource/Blender/Environment/FenwatchNoticeBoard/"
    "SM_EmbermereFenwatchNoticeBoard_01.fbx"
)
...
PLACEMENT_LOCATION = (-1560.0, -260.0, 0.0)
# Classic FBX mirrors Blender's local Y. This yaw faces the notice side toward
# the southwest village approach while keeping it offset from the travel lane.
PLACEMENT_YAW = -35.0
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch notice board integration failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 3684:
        fail("expected 3684 imported triangles, found {}".format(triangle_count))

    box_count = collision_box_count(mesh)
    if box_count != 3:
        fail("expected 3 imported collision boxes, found {}".format(box_count))

    bounds = mesh.get_bounds()
    dimensions = bounds.box_extent * 2.0
    expected = (286.0, 93.927, 277.0)
    ...
    removed, created = place_notice_board(mesh)
    unreal.log("Embermere Fenwatch notice board import paths: {}".format(imported_paths))
    unreal.log(
        "Embermere Fenwatch notice board material assignments: {}".format(assignments)
    )
    unreal.log(
        "Embermere Fenwatch notice board integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
            triangle_count,
            box_count,
            removed,
            created,
