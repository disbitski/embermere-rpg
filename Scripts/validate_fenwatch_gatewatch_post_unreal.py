"""Validate the saved Fenwatch gatewatch-post asset and map integration."""
...
PLACEMENT_LABEL = "Embermere_FenwatchGatewatchPost_EastRoad_01"
SIGNPOST_LABEL = "Embermere_RoadSignpost_01"
WAYSTONE_LABEL = "Embermere_Waystone_Road_01"
ROAD_GATE_LABEL = "Embermere_RoadGate_01"
...
EXPECTED_LOCATION = unreal.Vector(220.0, -650.0, 0.0)
EXPECTED_YAW = 20.0
EXPECTED_MATERIALS = {
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch gatewatch-post validation failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if mesh.get_num_triangles(0) != 5488:
        fail("triangle count drifted: {}".format(mesh.get_num_triangles(0)))
    total_collision, box_count = collision_counts(mesh)
    if total_collision != 7 or box_count != 7:
        fail(
            "expected exactly seven authored box colliders, found {} total / {} boxes".format(
                total_collision,
                box_count,
            )
        )
    ...
    expected_dimensions = (482.0, 327.0, 414.0)
    actual_dimensions = (
    ...
    if abs(float(bounds.origin.z) - 207.0) > 1.0:
    ...
    for actor in actor_subsystem.get_all_level_actors():
        ...
        if label == PLACEMENT_LABEL:
            ...
            context[label] = actor
    if len(placements) != 1:
        fail("expected one saved gatewatch post, found {}".format(len(placements)))
    ...
    distances = {
        label: horizontal_distance(
            EXPECTED_LOCATION,
            context[label].get_actor_location(),
        )
        for label in required_context
    }
    minimums = {
        SIGNPOST_LABEL: 500.0,
        WAYSTONE_LABEL: 1050.0,
        ROAD_GATE_LABEL: 1400.0,
        WORKSHOP_LABEL: 950.0,
        MARA_LABEL: 2200.0,
        PLAYER_START_LABEL: 2600.0,
    }
    for label, minimum in minimums.items():
        if distances[label] < minimum:
            fail(
                "gatewatch post moved too close to {}: {:.1f} cm".format(
                    label,
                    distances[label],
                )
            )

    road_centerline_clearance = point_to_segment_distance_2d(
        (EXPECTED_LOCATION.x, EXPECTED_LOCATION.y),
        (-900.0, -95.0),
        (1080.0, 540.0),
    )
    if road_centerline_clearance < 850.0:
        fail(
            "gatewatch post encroaches on the east road centerline: {:.1f} cm".format(
                road_centerline_clearance
            )
        )

    unreal.log(
        "Embermere Fenwatch gatewatch-post validation passed: "
        "482 x 327 x 414 cm, 5488 triangles, five shared materials, seven "
        "authored platform/step/wall/post collision boxes, one grounded "
        "project-owned presentation-only placement, {:.1f} cm road-centerline "
        "clearance, and clear signpost, waystone, gate, workshop, Mara, and "
        "PlayerStart spacing".format(road_centerline_clearance)
    )
