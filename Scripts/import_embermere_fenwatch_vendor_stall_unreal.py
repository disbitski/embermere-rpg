"""Import and place Embermere's project-owned Fenwatch vendor stall."""
...
SOURCE_FBX = (
    PROJECT_ROOT
    / "ArtSource/Blender/Environment/FenwatchVendorStall/"
    "SM_EmbermereFenwatchVendorStall_01.fbx"
)
...
PLACEMENT_LOCATION = (-1530.0, -1430.0, 0.0)
# FBX conversion mirrors Blender's local Y. Rotate the placement so the
# authored counter faces the quartermaster and customer approach.
PLACEMENT_YAW = 180.0
...
def fail(message):
    unreal.log_error("Embermere Fenwatch vendor stall integration failed: {}".format(message))
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 5476:
        fail("expected 5476 imported triangles, found {}".format(triangle_count))

    box_count = collision_box_count(mesh)
    if box_count != 5:
        fail("expected 5 imported collision boxes, found {}".format(box_count))

    bounds = mesh.get_bounds()
    dimensions = bounds.box_extent * 2.0
    expected = (394.0, 242.0, 306.0)
    ...
    removed, created = place_vendor_stall(mesh)
    unreal.log("Embermere Fenwatch vendor stall import paths: {}".format(imported_paths))
    unreal.log("Embermere Fenwatch vendor stall material assignments: {}".format(assignments))
    unreal.log(
        "Embermere Fenwatch vendor stall integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
            triangle_count,
            box_count,
            removed,
            created,
