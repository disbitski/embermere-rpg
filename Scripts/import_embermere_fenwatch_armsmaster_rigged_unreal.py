"""Import the rigged Fenwatch armsmaster and switch its art-only wrapper.
...
accepted static mesh as a reversible fallback, and leaves all trainer authority
...
SKELETAL_SOURCE = SOURCE_ROOT / "SK_EmbermereFenwatchArmsmaster_01.fbx"
IDLE_SOURCE = (
    SOURCE_ROOT / "Animations/A_EmbermereFenwatchArmsmaster_Idle.fbx"
)
DESTINATION_PATH = "/Game/Art/Embermere/Characters/NPCs/FenwatchArmsmaster"
STATIC_ASSET_PATH = DESTINATION_PATH + "/SM_EmbermereFenwatchArmsmaster_01"
SKELETAL_ASSET_NAME = "SK_EmbermereFenwatchArmsmaster_01"
...
IDLE_ASSET_NAME = "A_EmbermereFenwatchArmsmaster_Idle"
...
def fail(message):
    unreal.log_error(
        "Embermere rigged Fenwatch armsmaster integration failed: {}".format(
            message
        )
    )
    sys.exit(1)
...
def import_idle(skeleton):
    ...
    if idle.get_editor_property("skeleton") != skeleton:
        fail("Idle animation does not reference the imported armsmaster Skeleton")
...
def validate_assets(skeletal_mesh, skeleton, idle):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(PHYSICS_ASSET_PATH):
        fail("presentation-only armsmaster unexpectedly generated a PhysicsAsset")

    play_length = float(idle.get_play_length())
    if abs(play_length - 3.2) > 0.12:
...
def configure_saved_presentation(static_mesh, skeletal_mesh, idle):
    ...
    if len(presentations) != 1:
        fail("expected one armsmaster presentation, found {}".format(len(presentations)))
    if len(services) != 1:
        fail("expected one trainer service, found {}".format(len(services)))

    presentation = presentations[0]
    service = services[0]
    if not isinstance(presentation, unreal.EmbermereNpcPresentationActor):
        fail("saved armsmaster uses the wrong presentation class")
    if not isinstance(service, unreal.EmbermereTrainerServiceActor):
        fail("saved trainer service uses the wrong gameplay class")
    ...
    if (
        ...
        fail("saved armsmaster did not resolve through the skeletal lane")
    if (
        presentation.get_resolved_animation_mode()
        != unreal.EmbermereNpcAnimationMode.SINGLE_NODE_IDLE
    ):
        fail("saved armsmaster did not resolve the single-node Idle lane")
    ...
    if service.get_component_by_class(unreal.StaticMeshComponent):
        fail("trainer service unexpectedly owns static art")
    if service.get_component_by_class(unreal.SkeletalMeshComponent):
        fail("trainer service unexpectedly owns skeletal art")
...
def main():
    ...
    unreal.log(
        "Embermere rigged Fenwatch armsmaster integration passed: existing_before={}, skeletal_imports={}, idle_imports={}, materials={}, dimensions={}, Idle={:.3f}s, presentation={}, service={}, static_fallback={}".format(
            existing_before,
            skeletal_imports,
            idle_imports,
            assignments,
