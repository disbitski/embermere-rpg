"""Import Embermere's project-owned Fenwatch hitching trough."""
...
SOURCE_FBX = (
    ...
    "SM_EmbermereFenwatchHitchingTrough_01.fbx"
)
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch hitching-trough integration failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 3876:
        fail("expected 3876 imported triangles, found {}".format(triangle_count))
    ...
    unreal.log(
        "Embermere Fenwatch hitching-trough import paths: {}".format(imported_paths)
    )
    unreal.log(
        "Embermere Fenwatch hitching-trough material assignments: {}".format(
            assignments
        )
    )
    unreal.log(
        "Embermere Fenwatch hitching-trough asset import passed: "
