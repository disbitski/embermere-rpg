"""Import and place Embermere's modular boundary fence beside the road gate."""
...
SOURCE_FBX = PROJECT_ROOT / "ArtSource/Blender/Environment/BoundaryFence/SM_EmbermereBoundaryFence_01.fbx"
...
# The fence centers extend the gate's local +/-Y span at the same yaw. Their
...
def fail(message):
    unreal.log_error("Embermere boundary fence integration failed: {}".format(message))
    sys.exit(1)
...
def place_fences(mesh):
    ...
    created = []
    for label, location, yaw in PLACEMENTS:
...
def main():
    ...
    if abs(float(bounds.origin.z) - 90.0) > 2.0 or abs(float(bounds.box_extent.z) - 90.0) > 2.0:
        fail("unexpected imported Z bounds origin={} extent={}".format(bounds.origin.z, bounds.box_extent.z))

    removed, created = place_fences(mesh)
    unreal.log("Embermere boundary fence import paths: {}".format(imported_paths))
    unreal.log("Embermere boundary fence material assignments: {}".format(assignments))
    unreal.log(
        "Embermere boundary fence integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
