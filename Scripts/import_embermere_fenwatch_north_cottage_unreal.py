"""Import and place Embermere's project-owned Fenwatch north cottage."""
...
SOURCE_FBX = (
    ...
    "SM_EmbermereFenwatchNorthCottage_01.fbx"
)
...
PLACEMENT_LOCATION = (-2500.0, 680.0, 0.0)
# Classic FBX mirrors Blender's local Y. This yaw turns the covered porch back
# toward the village while preserving a useful lane between the two cottages.
PLACEMENT_YAW = 28.0
...
def fail(message):
    unreal.log_error("Embermere Fenwatch north cottage integration failed: {}".format(message))
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 7132:
        fail("expected 7132 imported triangles, found {}".format(triangle_count))
    ...
    removed, created = place_cottage(mesh)
    unreal.log("Embermere Fenwatch north cottage import paths: {}".format(imported_paths))
    unreal.log("Embermere Fenwatch north cottage material assignments: {}".format(assignments))
    unreal.log(
        "Embermere Fenwatch north cottage integration passed: triangles={}, collision boxes={}, replaced_prior={}, created={}".format(
            triangle_count,
            box_count,
            removed,
            created,
