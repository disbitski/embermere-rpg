"""Import and place Embermere's original road gate in the starter map."""
...
SOURCE_FBX = PROJECT_ROOT / "ArtSource/Blender/Environment/RoadGate/SM_EmbermereRoadGate_01.fbx"
...
PLACEMENT_LABEL = "Embermere_RoadGate_01"
PLACEMENT_LOCATION = (1080.0, 540.0, 20.0)
PLACEMENT_YAW = 20.0
...
def fail(message):
    unreal.log_error("Embermere road gate integration failed: {}".format(message))
    sys.exit(1)
...
def main():
    ...
    if abs(float(bounds.origin.z) - 186.865) > 2.0 or abs(float(bounds.box_extent.z) - 186.865) > 2.0:
        fail("unexpected imported Z bounds origin={} extent={}".format(bounds.origin.z, bounds.box_extent.z))

    removed, created = place_gate(mesh)
    unreal.log("Embermere road gate import paths: {}".format(imported_paths))
    unreal.log("Embermere road gate material assignments: {}".format(assignments))
    unreal.log(
        "Embermere road gate integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
