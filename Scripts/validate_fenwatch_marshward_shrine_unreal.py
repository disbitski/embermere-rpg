"""Validate the saved Fenwatch north-road marshward shrine."""
...
ASSET_PATH = (
    ...
    "SM_EmbermereFenwatchMarshwardShrine_01"
)
PLACEMENT_LABEL = "Embermere_FenwatchMarshwardShrine_NorthRoad_01"
EXPECTED_LOCATION = unreal.Vector(300.0, 1450.0, 0.0)
EXPECTED_YAW = -90.0
CONTEXT_LABELS = {
    "Embermere_RoadGate_01": 900.0,
    "Embermere_FenwatchGatewatchPost_EastRoad_01": 900.0,
    "Embermere_FenwatchHerbGarden_NorthCommons_01": 1600.0,
    "Quest_Giver_Mara_Fenwatch": 3000.0,
    "PlayerStart_Embermere_Village": 2800.0,
}
EXPECTED_MATERIALS = {
...
def fail(message):
    unreal.log_error("Embermere Fenwatch marshward-shrine validation failed: {}".format(message))
    sys.exit(1)
...
def main():
    ...
    if mesh.get_num_triangles(0) != 2648:
    ...
    expected_dimensions = (250.0, 183.238, 258.0)
    ...
    if abs(float(bounds.origin.z) - 129.0) > 1.0:
    ...
    if len(placements) != 1:
        fail("expected one saved marshward shrine, found {}".format(len(placements)))
    ...
    for context_label, minimum in CONTEXT_LABELS.items():
        ...
        if distance < minimum:
            fail("marshward shrine moved too close to {}: {:.1f} cm".format(context_label, distance))

    unreal.log(
        "EMBERMERE_FENWATCH_MARSHWARD_SHRINE_VALIDATION_SUCCESS: "
        "250 x 183.238 x 258 cm, 2648 triangles, five shared materials, four "
        "authored plinth/backstone/post boxes, one grounded presentation-only "
        "placement, and protected north-road spacing {}".format(distances)
    )
