"""Import Embermere's project-owned Fenwatch gatewatch post."""
...
SOURCE_FBX = (
    ...
    "SM_EmbermereFenwatchGatewatchPost_01.fbx"
)
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch gatewatch-post integration failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 5488:
        fail("expected 5488 imported triangles, found {}".format(triangle_count))

    box_count = collision_box_count(mesh)
    if box_count != 7:
        fail("expected 7 imported collision boxes, found {}".format(box_count))

    bounds = mesh.get_bounds()
    dimensions = bounds.box_extent * 2.0
    expected = (482.0, 327.0, 414.0)
    ...
    unreal.log(
        "Embermere Fenwatch gatewatch-post import paths: {}".format(imported_paths)
    )
    unreal.log(
        "Embermere Fenwatch gatewatch-post material assignments: {}".format(
            assignments
        )
    )
    unreal.log(
        "Embermere Fenwatch gatewatch-post asset import passed: "
        "triangles={}, collision boxes={}".format(
            triangle_count,
            box_count,
        )
    )
