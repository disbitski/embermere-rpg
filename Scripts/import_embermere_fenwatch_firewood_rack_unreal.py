"""Import and place Embermere's project-owned Fenwatch firewood rack."""
...
SOURCE_FBX = (
    ...
    "SM_EmbermereFenwatchFirewoodRack_01.fbx"
)
...
PLACEMENT_LABEL = "Embermere_FenwatchFirewoodRack_CottageWest_01"
PLACEMENT_LOCATION = (-3050.0, -120.0, 0.0)
# The rack follows the west cottage's roof line while opening its log face
# toward the village. It remains north of the handcart and west of traversal.
PLACEMENT_YAW = 38.0
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch firewood-rack integration failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if triangle_count != 3644:
        fail("expected 3644 imported triangles, found {}".format(triangle_count))
    ...
    expected = (392.937, 157.793, 238.0)
    ...
    if abs(float(bounds.origin.z) - 119.0) > 1.0:
        fail("unexpected imported Z origin {}".format(bounds.origin.z))

    removed, created = place_firewood_rack(mesh)
    unreal.log(
        "Embermere Fenwatch firewood-rack import paths: {}".format(imported_paths)
    )
    unreal.log(
        "Embermere Fenwatch firewood-rack material assignments: {}".format(
            assignments
        )
    )
    unreal.log(
        "Embermere Fenwatch firewood-rack integration passed: triangles={}, collision boxes={}, removed={}, created={}".format(
            triangle_count,
            box_count,
            removed,
            created,
