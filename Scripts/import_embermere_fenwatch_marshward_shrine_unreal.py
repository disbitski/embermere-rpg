"""Import Embermere's project-owned Fenwatch marshward shrine."""
...
SOURCE_FBX = (
    PROJECT_ROOT
    / "ArtSource/Blender/Environment/FenwatchMarshwardShrine/"
    "SM_EmbermereFenwatchMarshwardShrine_01.fbx"
)
...
def fail(message):
    unreal.log_error(
        "Embermere Fenwatch marshward-shrine import failed: {}".format(message)
    )
    sys.exit(1)
...
def main():
    ...
    if mesh.get_num_triangles(0) != 2648:
        fail("expected 2648 triangles, found {}".format(mesh.get_num_triangles(0)))
    if collision_box_count(mesh) != 4:
        fail(
            "expected four authored collision boxes, found {}".format(
                collision_box_count(mesh)
            )
        )
    bounds = mesh.get_bounds()
    dimensions = bounds.box_extent * 2.0
    expected = (250.0, 183.238, 258.0)
    ...
    unreal.log("Embermere Fenwatch marshward-shrine assignments: {}".format(assignments))
    unreal.log(
        "EMBERMERE_FENWATCH_MARSHWARD_SHRINE_IMPORT_SUCCESS: "
        "2648 triangles, four collision boxes, five shared materials"
    )
