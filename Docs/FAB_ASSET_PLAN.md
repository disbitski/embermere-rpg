...
- Validation rejects any `FabPass_` actor with meaningful pitch or roll and requires all nine original placements, their exact meshes/tags/transforms, and clean classic-FBX bounds plus authored box colliders for the imported Blender props. It also locks the two foliage transforms that reveal the accepted south-fence silhouette. `Scripts/validate_road_boundary_traces_unreal.py` separately proves three road-gate lanes clear, one gate support solid, both boundary-fence centers solid, and both boundary-stone cores solid.
