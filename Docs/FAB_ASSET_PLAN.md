...
- We have a first local environment pass using imported Epic/Fab packs plus a
  ...
  A matching hitching and feed trough now completes that working-yard cluster
  with a solid broad body and rail while keeping feed, feet, bands, rings,
  caps, braces, trim, and crest decorative and every surrounding lane open.
  ...
  A project-owned marshward shrine now marks the north-road transition to the
  Prowler pocket. Its plinth, backstone, and posts are solid; roof, hanging
  ward, offering bowl, talismans, trim, and crest are decorative. It has no
  gameplay authority and leaves the road centerline and shoulder open.
  A separate project-owned ancient carved relief now gives the blank west
  face of the first-Prowler ruin wall a legible identity. The relief itself is
  `NoCollision`; the original vendor wall retains all structural collision.
  ...
  gameplay actors do not change the 32-piece art baseline.
  We still do
...
- The pass removes the old visual-only village blockout buildings, road
  ...
  communal-well/gatewatch-post/herb-garden/marshward-shrine/ancient-ruin-relief family, Mara's non-colliding
  Fenwatch keeper, the non-colliding Fenwatch quartermaster and armsmaster,
  plus four visual-only marsh-reed clusters, for 32 original-art placements.
...
- Validation rejects any `FabPass_` actor with meaningful pitch or roll and
  requires all 32 original placements, their exact meshes/tags/transforms, the
  ...
  training-yard transform, project-owned tag, spacing from the dummy,
  armsmaster, and road pine, and replacement of
  `FabPass_Village_Fence_03`. Its initialized-world trace validator proves the
  ...
  `Scripts/validate_fenwatch_communal_well_unreal.py` locks the south-commons
  well's `348 x 220 x 322` cm bounds, 6,760 triangles, five shared materials,
  six authored curb/upright colliders, exact transform, project-owned tag,
  protected neighbor spacing, and presentation-only ownership. Its
  ...
  `Scripts/validate_fenwatch_hitching_trough_unreal.py` locks the working-yard
  trough's `367.15 x 133.903 x 189` cm bounds, 3,876 triangles, five shared
  materials, two authored body/rail boxes, exact transform, project-owned tag,
  neighbor spacing, and presentation-only ownership. Its initialized-world
  trace validator proves those two purposeful surfaces solid, keeps feed,
